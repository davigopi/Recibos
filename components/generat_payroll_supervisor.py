...
class Generat_payroll:
    ...
    # ira criar duas lista de todos os gerentes clt e parceiros
    def columns_to_list_full_seller(self):
        self.list_seller_ata = self.table_full_ata[self.column_gerente]
        self.list_seller_ata = self.list_seller_ata.unique()
        self.list_seller_ata.sort()
        self.list_seller_weekly = self.table_full_weekly[self.column_gerente]
    ...
    # define se gerente/parceiro a primeira é pela ata entrea/cadastro
    ...
    def table_list_administradora_line_add_sum(self):
        ...
        for table, ata, list_admnimistradora_line, sums in self.list_administradora_line_sum:
            print(table)
            print(ata, list_admnimistradora_line, sums)
            print('+++++++++++++++++++++++++++')
    ...
    def table_convert_pdf(self):
        ...
        for (
            ...
            if begin is True:
                ...
                sum_all_comissao = locale.currency(sum_all_comissao, grouping=True)
                text = ''
                text += f'            O/A Funcionário/a {gerente}, '
        ...
        new_folder = os.path.join(folder_documnts, "Contra cheque")
        if not os.path.exists(new_folder):
            os.makedirs(new_folder)

        name_file = f'Contra cheque {gerente}.pdf'
...
if __name__ == '__main__':
    ...
    table_seller_single = table_full[
        table_full[column_gerente] == seller_single]
    ...
    generat_payroll.column_gerente = column_gerente
