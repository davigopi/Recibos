...
class Table_manip_value():
  ...
  def edit_data_column_all(self, name_colunms):
    ...
    '''Remover colunas'''
    list_columns = [
        'Index', 'dia pag.', '1P referencia', '1P periodo inicial',
        '1P periodo final', 'D+ referencia', 'D+ perido inicial',
        'D+ periodo final', 'FAT referencia', 'FAT perido inicial',
        'FAT periodo final'
    ]
    ...
    for key in range(quantity_line - 1):
      value = self.table.iloc[key][cargo]
      if isinstance(value, numbers.Number):
        if np.isnan(value):
          self.table = self.table.drop(key)
    # reiniciar index
    self.table.reset_index(drop=True, inplace=True)
    '''Excluir linhas que se repete tudo, menos tipo_pagamento'''
    quantity_line = self.table.shape[0]
    dt_pag = 'Dt pag. por'
    for key in range(quantity_line - 1, -1, -1):
      if key == 0:
        continue
      if (self.table.iloc[key][cargo] == self.table.iloc[key - 1][cargo]) and (
          self.table.iloc[key][admin] == self.table.iloc[key - 1][admin]) and (
          self.table.iloc[key][dt_pag] == self.table.iloc[key - 1][dt_pag]
      ):
        self.table = self.table.drop(key)
        # allSame = True  # noqa
        # for name in name_columns:
        #   if self.table.iloc[key][name] != self.table.iloc[key - 1][name]:
        #     allSame = False
        #     break
        # if allSame:
        #   self.table = self.table.drop(key)
  ...
  def add_column_day_week(self, name_colunms):
    ...
    for key in range(quantity_line):
      date1 = self.table.iloc[key][dt_entrega]
      date2 = self.table.iloc[key][dt_cad_adm]
      dates = [date1, date2]
